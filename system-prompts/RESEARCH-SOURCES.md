...
- **Lost in the Middle: How Language Models Use Long Contexts**, Nelson
  F. Liu, Kevin Lin, John Hewitt, Ashwin Paranjape, Michele Bevilacqua,
  Fabio Petroni, Percy Liang (2023). arXiv:2307.03172,
  https://arxiv.org/abs/2307.03172 *(title, all seven authors and dates
  fetched from arXiv's own API and verified 2026-08-21; v1 2023-07-06,
  v3 2023-11-20)*
  *Added 2026-08-21. The mechanical counterpart to the "agent anxiety"
  framing this section is named after, and the reason that framing does
  not need to be read as emotional. Retrieval accuracy follows a U-shape
  across position: models use information at the start and end of a long
  context well and degrade sharply on the same information placed in the
  middle, well inside the advertised window. So a prompt does not fail
  only when it exceeds the limit, it degrades continuously as it fills,
  and the material most likely to be lost is whatever sits in the middle.
  That is an argument for externalising state rather than carrying it,
  which is what this estate's own store was built for: see
  `SECOND.BRAIN/.../Brain/llm_cognitive_wellbeing_and_zen_garden_2026_06_27.xml`
  (2026-06-27), which reached the same conclusion from observation and
  called it "mathematical anxiety". Older than the rest of this section,
  and load-bearing for the parts of the harness that decide what to keep
  in context versus what to write down and re-read.*
...
*Addition 2026-08-21: Liu et al., "Lost in the Middle" (arXiv:2307.03172)
added to the token-cost section. Title, authors and both version dates were
fetched from `export.arxiv.org/api/query` and compared before the entry was
written, per the standard the rest of this file is held to.*
