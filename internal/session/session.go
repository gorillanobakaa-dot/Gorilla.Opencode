...
type Session struct {
	...
	Title           string
	MessageCount    int64
	// PromptTokens and CompletionTokens are the CURRENT context occupancy —
	// how full the model's window is right now. Assigned, not accumulated, and
	// reset by compaction, because that is what the context gauge needs.
	PromptTokens     int64
	CompletionTokens int64
	// CumulativePromptTokens and CumulativeCompletionTokens are what the whole
	// conversation has cost, which is a different number and was previously
	// unavailable. The export and the sidebar were reading the two fields
	// above as lifetime totals and reporting the last turn only.
...
func (s *service) Save(ctx context.Context, session Session) (Session, error) {
	dbSession, err := s.q.UpdateSession(ctx, db.UpdateSessionParams{
		...
		CumulativePromptTokens:     session.CumulativePromptTokens,
		CumulativeCompletionTokens: session.CumulativeCompletionTokens,
...
func (s service) fromDBItem(item db.Session) Session {
	return Session{
		...
		CumulativePromptTokens:     item.CumulativePromptTokens,
		CumulativeCompletionTokens: item.CumulativeCompletionTokens,
