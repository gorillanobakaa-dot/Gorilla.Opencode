...
func TestCleanCatalogueDescriptionStripsMarkdownLinks(t *testing.T) {
	...
	got := CleanCatalogueDescription(in, 262144, 0.40, 2.00, true)
...
func TestCleanCatalogueDescriptionKeepsUsefulLength(t *testing.T) {
	...
	got := CleanCatalogueDescription(long, 262144, 0, 0, true)
	if len(got) < 120 {
...
func TestCleanCatalogueDescriptionHandlesEmpty(t *testing.T) {
	...
	got := CleanCatalogueDescription("", 0, 1.0, 2.0, true)
...
func TestCleanCatalogueDescriptionAlwaysStatesPrice(t *testing.T) {
	free := CleanCatalogueDescription("Some model", 1000, 0, 0, true)
	if !strings.HasPrefix(free, "FREE — ") {
		t.Errorf("free model must say FREE, got %q", free)
	}
	paid := CleanCatalogueDescription("Some model", 1000, 0.4, 2.0, true)
	...
	cheap := CleanCatalogueDescription("x", 1000, 0.04, 0.14, true)
	if !strings.Contains(cheap, "0.04") {
		t.Errorf("sub-dollar prices must keep two decimals, got %q", cheap)
	}
}

// The distinction this whole mechanism exists for: a verdict someone earned
// versus marketing copy nobody here has checked. Presenting the second as the
// first is how a picker starts lying quietly.
func TestUnverifiedDescriptionsAreMarked(t *testing.T) {
	vendor := CleanCatalogueDescription("Best model ever built", 1000, 1, 2, false)
	if !strings.Contains(vendor, "[unverified]") {
		t.Errorf("vendor prose must be marked, got %q", vendor)
	}
	ours := CleanCatalogueDescription("shit tier — cannot code", 1000, 1, 2, true)
	if strings.Contains(ours, "[unverified]") {
		t.Errorf("an earned verdict must NOT be marked unverified, got %q", ours)
	}
}
