...
func (q *Queries) ListSessionsByDir(ctx context.Context, startedIn string) ([]Session, error) {
	rows, err := q.query(ctx, q.listSessionsByDirStmt, listSessionsByDir, startedIn)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
